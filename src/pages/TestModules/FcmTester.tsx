...
import { Send, Copy, Check, ShieldCheck, AlertTriangle, Info, Zap, Globe, MessageSquare, Loader2 } from "lucide-react";
...
export default function FcmTester() {
  ...
  const applyTemplate = (templateId: string) => {
    ...
    if (template) {
      ...
      addLog(`Applied notification template: ${template.name}`, "info");
    }
  };
    ...
    <div className="flex-1 h-full flex flex-col min-w-0">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
        ...
        <div className="lg:col-span-4 space-y-6 overflow-y-auto pr-2 scrollbar-hide">
          <Card className="border border-slate-200 bg-white shadow-sm rounded-2xl overflow-hidden">
            ...
            <CardContent className="p-0">
              <div className="flex items-center justify-between p-4 bg-white/50 border-b border-slate-50">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-tight">Status</span>
                <Badge
                  variant="outline"
                  className={cn(
                    "font-bold border-none h-6",
                    permission === "granted" ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700",
                  )}
                >
              ...
              <div className="p-4 space-y-3 bg-white">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">FCM Token</span>
                  <button
                    onClick={copyToken}
                ...
                <div className="bg-slate-50/50 p-4 rounded-xl font-mono text-[10px] break-all h-28 overflow-y-auto border border-slate-100 text-slate-500 leading-relaxed scrollbar-hide">
              ...
              <div className="grid grid-cols-1 gap-1">
                {TEMPLATES.map((t) => (
                  <button
                    ...
                    className="flex items-center gap-4 p-3 rounded-xl border border-transparent hover:bg-slate-50 transition-all text-left group"
                  >
                    <div
                      className={cn(
                        "h-10 w-10 rounded-lg flex items-center justify-center shrink-0 transition-transform group-active:scale-95 shadow-sm",
                        t.bg,
                      )}
                    >
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-xs text-slate-900 uppercase tracking-tight">{t.name}</p>
                      <p className="text-[10px] text-slate-500 font-medium">{t.priority} Priority</p>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className="lg:col-span-8 flex flex-col h-full">
          <Card className="border border-slate-200 shadow-sm rounded-2xl overflow-hidden h-full flex flex-col bg-white">
            ...
            <CardContent className="p-8 space-y-6 flex-1 overflow-y-auto scrollbar-hide">
              <div className="grid grid-cols-1 gap-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-12 rounded-xl border-slate-100 bg-slate-50/50 focus:bg-white transition-all font-bold text-sm focus-visible:ring-primary/20"
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full min-h-[120px] p-4 rounded-xl border border-slate-100 bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-1 focus:ring-primary/20 transition-all font-medium text-sm resize-none"
              ...
              <div className="pt-4 mt-auto">
                <Button
                  ...
                  disabled={loading}
                  className="w-full h-16 bg-primary hover:bg-primary/90 text-primary-foreground rounded-2xl text-base font-black transition-all active:scale-[0.98] disabled:opacity-50 shadow-sm shadow-primary/10"
                >
                  {loading ? (
                    <>
